...
#python soundscape_provision.py --subscription $AZURE_SUBSCRIPTION_ID --name "soundscape-poc" --resource-group "soundscape" --keyvault-name "jtgTestKV" --location "uksouth" --scale "test" --service-version "1.0" --azure-container-registry-name "jtgacr" --container-registry-RG "containerregistrytest" --kubernetes-version "1.27.1"
...
#you need to install the AKS preview extention to Azure CLI
...
#'--set', 'controller.image.registry=mcr.microsoft.com/oss/kubernetes', Pulled this out because there is no function to add this to my container registry first so it fails
...
#    } This contains server hostname, port, dbname, user and password
    ...
    # It automatically uses the correct method based on the environment.
    # In a development environment, it uses the Azure CLI credentials.
    ...
    secret_client.set_secret(db_desc['name'], db_desc['dsn2'])
...
async def db_create_postgres(config, storage, sku, pgsub_id):
    ...
    dsn_template = 'postgresql://{user}:{password}@{host}:5432/osm'
    #dsn2_template = 'host={host} port=5432 dbname=osm user={user} password={password}'
    dsn = dsn_template.format(host=server_hostname,
                               user='osm',
                               password=password)
    proto = {
        'name': 'soundscape-database',
...
async def create_resources_async(config, mi_cluster, podsub_id, nodesub_id, container_registry_id, pgsub_id):
    ...
    return await asyncio.gather(
        ...
        db_create(config, pgsub_id) # This function returns a dictionary with db connection string
